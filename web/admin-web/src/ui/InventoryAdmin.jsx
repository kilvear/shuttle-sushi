...
import { inventory, menu } from '../api'
...
export default function InventoryAdmin(){
  ...
  useEffect(() => {
    ...
    async function tick(){
      try {
        const [storeIds, list, storeList, cat] = await Promise.all([
          inventory.stores().catch(()=>({ stores:['store-001']})),
          inventory.centralList(search, 200),
          inventory.stock(selectedStore).catch(()=>({ items: [] })),
          menu.list().catch(()=>({ items: [] }))
        ])
        if (!alive) return
        setStores(storeIds.stores || ['store-001'])
        setCentralRows(list.items || [])
        setStoreRows(storeList.items || [])
  ...
  const compareRows = useMemo(() => {
    const map = new Map()
    for (const r of filteredCentral) map.set(r.sku, { sku:r.sku, central:r.qty, store:0 })
    for (const r of filteredStore) {
      const cur = map.get(r.sku) || { sku:r.sku, central:0, store:0 }
      cur.store = r.qty
      map.set(r.sku, cur)
    }
    let list = Array.from(map.values()).map(x => ({...x, drift: (x.central - x.store)}))
    if (hideZero) list = list.filter(x => x.drift !== 0)
    return list.sort((a,b)=>a.sku.localeCompare(b.sku))
  }, [filteredCentral, filteredStore, hideZero])
  ...
  const skuOptions = useMemo(() => {
    ...
    for (const r of storeRows) if (r?.sku) set.add(r.sku)
    return Array.from(set).sort()
  }, [catalog, centralRows, storeRows])
  ...
  function exportCSV(){
    ...
    if (view === 'store') {
      ...
      for (const r of filteredStore.slice(storePage*pageSize, storePage*pageSize + pageSize)) rows.push([r.sku, nameBySku.get(r.sku)||'', r.qty])
      return downloadCSV(`store_inventory_${ts}.csv`, rows)
    }
    // compare
    const rows = [['sku','name','central_qty','store_qty','drift']]
    for (const r of compareRows) rows.push([r.sku, nameBySku.get(r.sku)||'', r.central, r.store, r.drift])
          ...
          {loading ? <div>Loading…</div> : (
            <div style={{ maxHeight:420, overflow:'auto' }}>
              <table width="100%" style={{ borderCollapse:'collapse' }}>
                <thead>
                  <tr><th align="left">SKU</th><th align="left">Name</th><th align="right">Qty (central)</th><th>Actions</th></tr>
                </thead>
                <tbody>
        ...
        loading ? <div>Loading…</div> : (
          <div style={{ maxHeight:420, overflow:'auto' }}>
            <table width="100%" style={{ borderCollapse:'collapse' }}>
              ...
              <tbody>
                {filteredStore.slice(storePage*pageSize, storePage*pageSize + pageSize).map(r => (
                  <tr key={r.sku}>
                    <td>{r.sku}</td>
                    <td>{nameBySku.get(r.sku) || ''}</td>
                    <td align="right">{r.qty}</td>
            ...
            <div style={{ marginTop:8 }}>
              ...
              <button onClick={()=>setStorePage(p=> (storePage+1)*pageSize < filteredStore.length ? p+1 : p)} disabled={(storePage+1)*pageSize >= filteredStore.length}>Next</button>
            </div>
          </div>
        )
      )}
      ...
      {view==='compare' && (
        loading ? <div>Loading…</div> : (
          <div style={{ maxHeight:420, overflow:'auto' }}>
            <table width="100%" style={{ borderCollapse:'collapse' }}>
              <thead>
                <tr><th align="left">SKU</th><th align="left">Name</th><th align="right">Central</th><th align="right">{selectedStore}</th><th align="right">Drift</th></tr>
              </thead>
              <tbody>
                {compareRows.map(r => (
                  <tr key={r.sku}>
